...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    unscaled_att_scores = torch.matmul(query,key.transpose(-1,-2))
    unscaled_att_scores = unscaled_att_scores + attention_mask

    # scale
    attention_scores = unscaled_att_scores / math.sqrt(self.attention_head_size)
    attention_probabilities = F.softmax(attention_scores, dim=-1)

    v_hat = torch.matmul(attention_probabilities, value)
    v_hat = v_hat.permute(0,2,1,3).contiguous()

    # bs, seq_len, num_attention_heads * attention_head_size
    v_hat = v_hat.view(v_hat.shape[0], v_hat.shape[1], self.all_head_size)
    return v_hat
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    att_out = self.self_attention(hidden_states, attention_mask)

    # add-norm layer
    att_out = self.add_norm(hidden_states, att_out, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    # feed forward
    ff_out = self.interm_dense(att_out)
    ff_out = self.interm_af(ff_out)

    # another add-norm layer
    ff_out = self.add_norm(att_out, ff_out, self.out_dense, self.out_dropout, self.out_layer_norm)
